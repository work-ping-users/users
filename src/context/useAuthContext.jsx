...
export function AuthProvider({ children }) {
  ...
  const login  = useCallback((token) => { if (token) setToken(token); return verifySession(); }, [verifySession]);
  const signUp = useCallback((token) => { if (token) setToken(token); return verifySession(); }, [verifySession]);
