...
const EmailItem = ({ createdAt, id, attachments, content, from, important, read, starred, subject, to, sent }) => {
  ...
  return (
    <li className={clsx(!read && 'unread')}>
      <div className="col-mail col-mail-1">
        ...
        <IconifyIcon
          icon={starred ? 'bxs:star' : 'bx:star'}
          className={clsx('star-toggle', {
            'text-warning': starred,
          })}
        />
        <IconifyIcon
          icon={important ? 'bxs:tag-alt' : 'bx:tag-alt'}
          className={clsx('important-toggle', {
      ...
      <div className="col-mail col-mail-2">
        <p role="button" onClick={() => changeActiveMail(id)} className="subject mb-0">
          ...
          {attachments?.slice(0, 3)?.map((attachment, idx) => (
            <AttachmentItem key={idx} {...attachment} />
          ))}
